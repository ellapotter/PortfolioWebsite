...
export function ElectionSystemProjectDetail({ project, caseStudy }: Props) {
  ...
  return <main className="bg-pink-50 pt-28 text-pink-950 md:pt-[4.75rem]"><article>
    ...
    <section className={`bg-surface ${sectionPadding}`}><div className={sectionContainer}><Heading label="Project overview" title={caseStudy.overview.heading} /><div className="grid gap-5 text-base leading-relaxed text-pink-800 lg:grid-cols-3">{caseStudy.overview.paragraphs.map(p => <p key={p}>{p}</p>)}</div></div></section>
    ...
    <section className={`bg-surface ${sectionPadding}`}><div className={sectionContainer}><Heading label="Interface" title="Gallery" /><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{details.gallery.map(image => <figure key={image.src} className="overflow-hidden rounded-3xl border border-pink-200 bg-surface shadow-sm"><div className="relative aspect-video bg-slate-100"><Image src={image.src} alt={image.alt} fill className="object-contain" sizes="(max-width: 768px) 100vw, 50vw" /></div><figcaption className="p-4 text-sm leading-relaxed text-pink-700">{image.caption}</figcaption></figure>)}</div></div></section>
