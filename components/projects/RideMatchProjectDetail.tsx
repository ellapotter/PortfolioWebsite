...
function Phone({ src, alt, priority = false, className = "" }: { src: string; alt: string; priority?: boolean; className?: string }) {
  return <div className={`mx-auto w-full max-w-[17rem] rounded-[2.25rem] border-[7px] border-pink-950 bg-background p-1.5 shadow-xl shadow-pink-300/40 ${className}`}><div className="relative aspect-[430/932] overflow-hidden rounded-[1.55rem] bg-slate-900"><Image src={src} alt={alt} fill priority={priority} className="object-contain" sizes="(max-width: 640px) 70vw, 18rem" /></div></div>;
}
...
export function RideMatchProjectDetail({ project, caseStudy }: Props) {
  ...
  return <main className="overflow-x-clip bg-pink-50 pt-28 text-pink-950 md:pt-[4.75rem]"><article>
    ...
    <section className={`bg-surface ${sectionPadding}`}><div className={sectionContainer}><Heading label="Core experience" title="From Account Creation to a Completed Ride" /><ol className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{details.flow.map((step, i) => <li key={step} className="rounded-2xl border border-pink-200 bg-pink-50 p-4"><span className="mb-3 flex h-8 w-8 items-center justify-center rounded-full bg-pink-500 font-mono text-xs font-bold text-white">{i + 1}</span><span className="text-sm leading-relaxed">{step}</span></li>)}</ol></div></section>
    ...
    <section className={`bg-surface ${sectionPadding}`}><div className={sectionContainer}><Heading label="Challenges and learning" title="Building Across Mobile, Web, API, and Data Layers" /><div className="grid gap-6 lg:grid-cols-3">{[
